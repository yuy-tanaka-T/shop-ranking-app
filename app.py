...
ROW_IDX_DATA_START = 17 # 18行目からデータ開始
...
if uploaded_file is not None:
    try:
        ...
        # 1. 日付の取得 (C13)
        try:
            raw_text = str(all_df.iloc[ROW_IDX_DATE, 2])
        ...
        # 2. 項目名の列位置を特定 (16行目)
        item_row = all_df.iloc[ROW_IDX_ITEM_NAME]
        found_items = {} 
        ...
        if gap > 0:
            ...
            for item_name, col_idx in found_items.items():
                try:
                    # 1. 係数 (9行目)
                    unit_pt_val = all_df.iloc[ROW_IDX_COEFF, col_idx]
            ...
            if not top_5_items:
                ...
                h_cols[3].markdown("**③不足数**")
                h_cols[4].markdown("**④向上Pt**")
